 ...
 */
/*!
 * \file scope/scope_check.hpp
 *
 * This header contains definition of \c scope_check template.
 */
...
// Workaround for clang < 5.0 which can't pass scope_check as a template template parameter from within scope_check definition
template< typename T >
using is_not_like_scope_check = detail::is_not_like< T, scope_check >;
 ...
 *
 * Additionally, the condition function object `operator()` must not throw,
 * as otherwise the action function object may not be called.
 *
...
public:
     ...
     *              `scope_check(std::forward< F >(func), Cond(), active)`.
     *
    ...
    template<
        ...
        , typename = typename std::enable_if< detail::conjunction<
            ...
            std::is_constructible<
                ...
                bool
            >,
            detail::is_not_like_scope_check< F >
        >::value >::type
        //! \endcond
    >
    explicit scope_check(F&& func, bool active = true)
    ...
    template<
        ...
        //! \endcond
    >
    explicit scope_check(F&& func, C&& cond, bool active = true)
        noexcept(BOOST_SCOPE_DETAIL_DOC_HIDDEN(
            std::is_nothrow_constructible<
                data,
                typename detail::move_or_copy_construct_ref< F, Func >::type,
    ...
    scope_check& operator= (scope_check&&) = delete;

    scope_check(scope_check const&) = delete;
    scope_check& operator= (scope_check const&) = delete;
...
template< typename Func, typename Cond >
scope_check(Func, Cond) -> scope_check< Func, Cond >;

template< typename Func, typename Cond >
scope_check(Func, Cond, bool) -> scope_check< Func, Cond >;

template< typename Func, typename Cond >
scope_check(scope_check< Func, Cond >&&) -> scope_check< Func, Cond >;
 ...
 *              `scope_check< std::remove_cvref_t< F >,
...
> make_scope_check(F&& func, C&& cond, bool active = true)
...
#endif // BOOST_SCOPE_SCOPE_CHECK_HPP_INCLUDED_
